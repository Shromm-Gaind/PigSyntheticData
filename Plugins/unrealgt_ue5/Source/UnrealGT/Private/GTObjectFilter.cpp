...
FGTObjectFilter::FGTObjectFilter()
    : StaticMesh(nullptr), SkeletalMesh(nullptr), WildcardMeshName(TEXT("")),
      ...
      ActorInstance(nullptr), SocketLocations() {}
  ...
  if (SkeletalMesh || !WildcardMeshName.IsEmpty()) {
    ...
    if (SkeletalMeshComponent) {
      if (SkeletalMesh && SkeletalMeshComponent->SkeletalMesh == SkeletalMesh) {
        bMatchesSkeletalMesh = true;
        // If this filter is meant for sockets, retrieve the socket locations
        if (bIsBoneSocket)
        {
          SocketLocations.Empty();  // Clear previous locations
          for (const FName& SocketName : SkeletalMeshComponent->GetAllSocketNames())
          {
              ...
              //SocketLocations.Add(SocketLocations);
              UE_LOG(
                  LogTemp,
                  Warning,
                  TEXT("Socket: %s, Location: (%f, %f, %f)"),
                  *SocketName.ToString(),
                  Location.X,
                  Location.Y,
                  Location.Z);
